      ...
      <div className="space-y-2">
        {filteredFolders.length > 0 ? (
           filteredFolders.map((folder) => {
              ...
              // Indicateur latéral (Status Line)
              ...
              return (
                  <Link key={folder.id} href={`/site/${folder.id}`} className="block group">
                      <Card className="relative overflow-hidden border border-gray-200 bg-white shadow-sm hover:shadow-md hover:border-blue-300/50 transition-all duration-200">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center py-3 pl-5 pr-4 gap-4">
                              
                              {/* 1. IDENTITÉ DU SITE (Compact) */}
                              <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-1.5 mt-1.5 text-xs text-gray-500 group-hover:text-gray-700">
                                      <Globe className="h-3 w-3" />
                                      <span className="truncate font-mono opacity-80">{folder.root_url}</span>
                                  </div>
                              </div>

                              {/* 2. BLOCS MÉTRIQUES "TECH" */}
                              <div className="flex items-center gap-2 sm:gap-4 border-t sm:border-t-0 border-gray-100 pt-3 sm:pt-0 overflow-x-auto no-scrollbar">
                                  
                                  {/* Bloc Santé */}
                                  <div className={cn("flex items-center gap-2 px-2.5 py-1.5 rounded border min-w-[90px] justify-between", scoreClass)}>
                                      <div className="flex items-center gap-1.5">
                                          <BarChart3 className="h-3.5 w-3.5 opacity-70" />
                                          <span className="text-[10px] font-bold uppercase tracking-wider">Score</span>
                                      </div>
                                      <span className="font-mono text-sm font-bold">{healthScore ?? '-'}</span>
                                  </div>

                                  {/* Bloc Pages */}
                                  <div className="flex items-center gap-2 px-2.5 py-1.5 rounded border border-gray-200 bg-gray-50 text-gray-600 min-w-[90px] justify-between">
                                      <div className="flex items-center gap-1.5">
                                          <Layers className="h-3.5 w-3.5 opacity-70" />
                                          <span className="text-[10px] font-bold uppercase tracking-wider">Pages</span>
                                      </div>
                                      <span className="font-mono text-sm font-bold">{pageCount}</span>
                                  </div>

                                  {/* Bloc SSL */}
                                  <div className={cn("flex items-center gap-2 px-2.5 py-1.5 rounded border min-w-[100px] justify-between", 
                                      ...
                                      <div className="flex items-center gap-1.5">
                                          {isSslOk ? <Lock className="h-3.5 w-3.5 opacity-70" /> : <Unlock className="h-3.5 w-3.5 opacity-70" />}
                                  ...
                                  <div className="hidden sm:flex text-gray-300 group-hover:text-blue-600 group-hover:translate-x-1 transition-all pl-2">
