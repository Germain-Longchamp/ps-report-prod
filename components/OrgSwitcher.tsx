...
import { CreateOrgModal } from "./CreateOrgModal" // <--- IMPORT
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="lg"
          className="w-full justify-between px-2 hover:bg-zinc-800 text-zinc-400 hover:text-white mb-6 h-12 border border-transparent hover:border-zinc-700"
        >
          <div className="flex items-center gap-3 text-left">
            ...
            <div className="grid flex-1 text-left text-sm leading-tight">
              ...
              <span className="truncate text-xs text-zinc-500">
                {organizations.length > 1 ? "Changer..." : "Plan Pro"}
              </span>
            </div>
          </div>
      ...
      <DropdownMenuContent className="w-[220px] bg-zinc-950 border-zinc-800 text-zinc-400 ml-2">
        <DropdownMenuLabel className="text-zinc-500 text-xs uppercase tracking-wider">Mes Organisations</DropdownMenuLabel>
        
        {organizations.map((org) => (
          <DropdownMenuItem
            key={org.id}
            onClick={() => switchOrganization(org.id.toString())}
            className="focus:bg-zinc-900 focus:text-white cursor-pointer flex items-center justify-between py-2"
          >
        ...
        <CreateOrgModal>
            <DropdownMenuItem 
                onSelect={(e) => e.preventDefault()} // Important pour ne pas fermer le dropdown immédiatement
                className="gap-2 p-2 focus:bg-zinc-900 focus:text-white cursor-pointer"
            >
