...
export default async function SiteDetailsPage({ params }: PageProps) {
  ...
  // MODIFICATION 1 : Récupérer TOUS les audits pour pouvoir filtrer ensuite
  ...
  // On isole le dernier audit GLOBAL (celui qui n'a pas de page_id, donc la racine) pour l'affichage du haut
  const lastMainAudit = allAudits?.find(a => a.page_id === null) || null;

  // Fonction helper pour trouver le dernier audit d'une page spécifique
  const getLastAuditForPage = (pageId: string) => {
    return allAudits?.find(a => a.page_id === pageId) || null
  }
      ...
      <div className="flex flex-col gap-6 border-b pb-8">
        <div className="flex items-start justify-between">
            ...
            <div className="flex gap-3">
                ...
                {/* Bouton pour l'audit global (Racine) */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card className="border-gray-200 shadow-sm flex flex-col justify-center p-6 h-full">
                ...
                <div className="text-xl font-bold text-gray-900 truncate">
                    {lastMainAudit?.https_valid ? "Sécurisé" : "Non sécurisé"}
                </div>
            ...
            <Card className="border-gray-200 shadow-sm flex flex-col justify-center p-6 h-full">
                ...
                <div className="text-xl font-bold text-gray-900">
                    {lastMainAudit?.indexable ? "Indexable" : "Bloqué"}
                </div>
      ...
      <section className="space-y-6 pt-6 border-t border-gray-100">
         ...
         <Card className="border-gray-200 shadow-sm bg-gray-50/50">
            <CardContent className="p-6 space-y-8">
                ...
                {/* MODIFICATION 2 : Utilisation de PageRow pour afficher la liste */}
                    ...
                    {pages && pages.length > 0 ? (
                        pages.map((page) => (
                            <PageRow 
                                ...
                                page={page} 
                                folderId={id} 
                                lastAudit={getLastAuditForPage(page.id)} 
                            />
                        ))
                    ) : (
